...
3.  **Set up environment variables**:
    - Add your model API keys, e.g. `GEMINI_API_KEY`, `DEEPSEEK_API_KEY`, `SILICONFLOW_API_KEY`
    - Add `DISCORD_TOKEN` if using Discord
    - Add email / Notion variables only if using those tools

4.  **Configure runtime**:
    - `config.json` is the active local config
...
```json
"shared_soul": "~/Documents/agent-kb/kb/SOUL.md",
"agent_profile": "~/Documents/agent-kb/kb/applied-scientist/INDEX.md",
"file_access": {
    "default_scope": "kb",
    "scopes": {
        "kb": {
            "roots": ["~/Documents/agent-kb/kb"],
            "read": true,
            "list": true
        }
    }
...
```bash
SWARM_SHARED_SOUL=~/Documents/agent-kb/kb/SOUL.md \
SWARM_AGENT_PROFILE=~/Documents/agent-kb/kb/applied-scientist/INDEX.md \
SWARM_KNOWLEDGE_ROOTS=~/Documents/agent-kb/kb \
python -m swarm_agent --debug
```
...
External knowledge is not a Swarm component. The KB lives at
`~/Documents/agent-kb/` and is exposed to Swarm through the `kb` file scope.
