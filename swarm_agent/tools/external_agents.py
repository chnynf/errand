...
async def invoke_external_agent(
    ...
    better suited to the task. The CLI receives a scoped prompt and Swarm
    ...
    if approved:
        ...
        response = {
            ...
            "response": {
                ...
                "response": {
                    "behavior": "deny",
                    "message": "User denied this tool request through Swarm.",
                },
            },
        }
...
def _build_external_prompt(task: str, context: str) -> str:
    user_name = os.getenv("SWARM_USER_NAME", "")
    user_email = os.getenv("SWARM_USER_EMAIL", "")
    identity_lines = []
    if user_name:
    ...
    parts = ["You are being invoked by Swarm as an external delegated agent."]
    if identity_lines:
    ...
    if context.strip():
        parts.extend(["", f"CONTEXT FROM SWARM:\n{context.strip()}"])
    return "\n".join(parts)
