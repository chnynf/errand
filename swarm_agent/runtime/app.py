"""SwarmApp: process lifecycle and component wiring."""
...
from swarm_agent.config import SwarmConfig, load_swarm_config
from swarm_agent.contracts.interfaces import SwarmInterface, UserMessage
from swarm_agent.runtime.debug import set_debug
...
class SwarmApp:
    ...
    def __init__(
        ...
        *,
        ...
        from swarm_agent.sessions import SessionManager
        ...
        self._interfaces: list[SwarmInterface] = []
    ...
    async def start(self) -> None:
        ...
        print(f"Swarm runtime started. Interfaces: {interface_names}{scheduler_suffix}")
    ...
    def _build_interfaces(self, names: list[str]) -> list[SwarmInterface]:
        interfaces: list[SwarmInterface] = []
        for name in names:
            if name == "discord":
                from swarm_agent.interfaces.discord_interface import DiscordInterface
                ...
                from swarm_agent.interfaces.cli import CliInterface

                interfaces.append(CliInterface(self, debug=self.debug))
            elif name == "web":
                from swarm_agent.interfaces.web import WebInterface
...
async def run_swarm(
    *,
    ...
    app = SwarmApp(debug=debug, interfaces=interfaces)
    await app.start()
