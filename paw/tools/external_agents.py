...
async def invoke_external_agent(
    ...
    better suited to the task. The CLI receives a scoped prompt and Paw
    ...
    if approved:
        ...
        response = {
            ...
            "response": {
                ...
                "response": {
                    "behavior": "deny",
                    "message": "User denied this tool request through Paw.",
                },
            },
        }
...
def _build_external_prompt(task: str, context: str) -> str:
    user_name = os.getenv("PAW_USER_NAME", "")
    user_email = os.getenv("PAW_USER_EMAIL", "")
    identity_lines = []
    if user_name:
    ...
    parts = ["You are being invoked by Paw as an external delegated agent."]
    if identity_lines:
    ...
    if context.strip():
        parts.extend(["", f"CONTEXT FROM PAW:\n{context.strip()}"])
    return "\n".join(parts)
